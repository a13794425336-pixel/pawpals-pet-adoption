...
export function normalizeBrowseFilter(value: string | undefined): BrowseFilterKey {
...
export async function getBrowseScreenData(search: string, filter: BrowseFilterKey) {
  const profile = await loadProfile();
...
export async function getPetDetailData(slug: string) {
